<template>
  <div :style="[bgColor, colorText]" class="py-12">
    <UContainer>
      <h1 class="fm3 uppercase mb-10" v-html="app.register_steps_title_hotsite"></h1>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 justify-center">
        <div v-for="(step, index) in app.register_steps_list_items_hotsite.list" class="flex flex-col items-center">
          <div :style="[colorBgButton, colorTextButton]" class="w-10 h-10 rounded-full flex justify-center items-center">
            <span class="fm3">{{ index + 1 }}</span>
          </div>

          <h2 class="fm3 mt-3 text-2xl">{{ step.one }}</h2>
          <p class="fm1 mt-5 text-center text-base leading-5">{{ step.two }}</p>
        </div>
      </div>
    </UContainer>
  </div>
